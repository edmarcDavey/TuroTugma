  ...
  <div class="max-w-full mx-auto">
    ...
    <script>
      ...
      window.schedulerData = {
        subjects: JSON.parse('<?php echo addslashes(json_encode($subjects)); ?>'),
        teachers: JSON.parse('<?php echo addslashes(json_encode($teachers)); ?>'),
        periodsRegular: JSON.parse('<?php echo addslashes(json_encode($periodsRegular)); ?>'),
        periodsShortened: JSON.parse('<?php echo addslashes(json_encode($periodsShortened)); ?>'),
        sections: JSON.parse('<?php echo addslashes(json_encode($sections)); ?>'),
        specializedSubjectCodes: ['SPA', 'SPJ'], // Specialized subjects for special sections
        generatedSchedule: JSON.parse('<?php echo addslashes(json_encode($generatedSchedule ?? [])); ?>'), // Real schedule data
        days: JSON.parse('<?php echo addslashes(json_encode($days ?? [1,2,3,4,5])); ?>'), // Days for section schedule
        // Build teacher ancillary assignments map for restriction checking
        teacherAncillaries: (function() {
          const map = {};
          const teachers = JSON.parse('<?php echo addslashes(json_encode($teachers)); ?>');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadExistingSchedule(level = 'junior_high') {
    fetch(`<?php echo e(route('admin.schedule-maker.get-latest-schedule')); ?>?level=${level}`)
      .then(response => response.json())
      .then(data => {
