  ...
  <div class="max-w-full mx-auto">
    ...
    <div id="tab-qualifications" class="settings-tab-content hidden p-6">
      ...
      <div class="border rounded-lg p-6 bg-white shadow">
        <form method="POST" action="{{ route('admin.schedule-maker.settings.save') }}" class="space-y-6">
          @csrf
          <input type="hidden" name="level" value="general" />
            ...
            <table class="min-w-full text-sm text-slate-800">
              ...
              <tbody class="divide-y">
                @foreach($teachers as $teacher)
                  <tr class="align-top">
                    <td class="py-3 px-3 font-medium">{{ $teacher->name }}</td>
                    <td class="py-3 px-3">
                      <select name="teacher_subjects[{{ $teacher->id }}][]" class="input w-full" multiple size="5">
                    ...
                    <td class="py-3 px-3">
                      <select name="teacher_sections[{{ $teacher->id }}][]" class="input w-full" multiple size="5">
                        <optgroup label="Junior High">
                          @foreach($sections_jhs as $section)
                            <option value="{{ $section->id }}">{{ $section->name }}</option>
                          @endforeach
                        </optgroup>
                        <optgroup label="Senior High">
                          @foreach($sections_shs as $section)
                            <option value="{{ $section->id }}">{{ $section->name }}</option>
                          @endforeach
                        </optgroup>
                      </select>
