  ...
  <div class="grid grid-cols-4 gap-4">
    <div class="col-span-1">
      <div class="flex items-center justify-between mb-2">
        ...
        @endphp
        <h2 class="text-lg font-semibold">Teachers ({{ $teacherCount }})</h2>
      ...
      <form method="GET" class="mb-3" id="teacher-search-form" onsubmit="return false;">
        <div class="flex gap-2 mb-2">
          <input id="teacher-search-input" name="q" value="{{ old('q', $q ?? request('q')) }}" placeholder="Search by name or staff ID" class="block w-full border border-gray-300 rounded-md p-2 h-9" autocomplete="off" />
        ...
        <div class="space-y-2">
          <select id="filter-designation" name="designation" class="block w-full border border-gray-300 rounded-md p-2 h-9 text-sm bg-white">
            ...
            @foreach($designations ?? [] as $designation)
              <option value="{{ $designation }}" @if(($filterDesignation ?? '') === $designation) selected @endif>{{ $designation }}</option>
            @endforeach
          </select>
          ...
          <select id="filter-subject" name="subject" class="block w-full border border-gray-300 rounded-md p-2 h-9 text-sm bg-white">
            ...
            @foreach($subjects as $subject)
              <option value="{{ $subject->id }}" @if(($filterSubject ?? '') == $subject->id) selected @endif>{{ $subject->name }}</option>
            @endforeach
          </select>
          ...
          <select id="filter-grade-level" name="grade_level" class="block w-full border border-gray-300 rounded-md p-2 h-9 text-sm bg-white">
            ...
            @foreach($gradeLevels as $gradeLevel)
              <option value="{{ $gradeLevel->id }}" @if(($filterGradeLevel ?? '') == $gradeLevel->id) selected @endif>{{ $gradeLevel->name }}</option>
            @endforeach
          </select>
