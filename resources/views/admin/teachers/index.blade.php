...
@section('content')
  <div class="grid grid-cols-4 gap-4">
    <div class="col-span-1">
      <div class="flex items-center justify-between mb-2">
        ...
        @endphp
        <h2 class="text-lg font-semibold">Teachers (<span id="teacher-count">{{ $teacherCount }}</span>)</h2>
      ...
      <form method="GET" class="mb-3" id="teacher-search-form" onsubmit="return false;">
        <div class="mb-2">
          <div class="relative">
            ...
            <input id="teacher-search-input" name="q" value="{{ old('q', $q ?? request('q')) }}" placeholder="Search by name or staff ID" class="block w-full border border-gray-300 rounded-md p-2 h-9 pl-10" autocomplete="off" />
          </div>
        </div>
            ...
            <div id="designation-dropdown" class="custom-dropdown" tabindex="0">
              ...
              <div class="dropdown-menu hidden" id="designation-menu">
                @foreach($designations ?? [] as $designation)
                  <div class="dropdown-item" data-value="{{ $designation }}" data-label="{{ $designation }}">{{ $designation }}</div>
                @endforeach
              </div>
            </div>
          </div>
          <div class="relative mb-2">
            <div id="status-dropdown" class="custom-dropdown" tabindex="0">
              ...
              <div class="dropdown-menu hidden" id="status-menu">
                @foreach(config('teachers.statuses', []) as $status)
                  <div class="dropdown-item" data-value="{{ $status }}" data-label="{{ $status }}">{{ $status }}</div>
                @endforeach
              </div>
            </div>
          </div>
            ...
            <div id="subject-dropdown" class="custom-dropdown" tabindex="0">
              ...
              <div class="dropdown-menu hidden" id="subject-menu">
                @foreach($subjects as $subject)
                  <div class="dropdown-item" data-value="{{ $subject->id }}" data-label="{{ $subject->name }}">{{ $subject->name }}</div>
                @endforeach
              </div>
            </div>
          </div>
            ...
            <div id="grade-dropdown" class="custom-dropdown" tabindex="0">
              ...
              <div class="dropdown-menu hidden" id="grade-menu">
                @foreach($gradeLevels as $gradeLevel)
                  <div class="dropdown-item" data-value="{{ $gradeLevel->id }}" data-label="{{ $gradeLevel->name }}">{{ $gradeLevel->name }}</div>
                @endforeach
              </div>
            </div>
          </div>
      ...
      <div class="overflow-y-auto" style="max-height:calc(100vh - 140px);">
        <ul id="teacher-list" class="space-y-2">
          @include('admin.teachers._list', ['teachers' => $teachers])
        </ul>
      </div>

      <div class="mt-4" id="teacher-pagination" style="display:block">{{ $teachers->links() }}</div>
    </div>
    ...
    <div id="teachers-urls" style="display:none"
      data-fragment-url="{{ url('admin/teachers/fragment') }}"
      data-base-url="{{ url('admin/teachers') }}"
      data-list-url="{{ url('admin/teachers/list') }}"
  ...
  <div id="import-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4">
      ...
      <form id="import-form" method="POST" action="{{ route('admin.teachers.import') }}" enctype="multipart/form-data">
        @csrf
        <div class="mb-4">
        ...
        <div class="flex gap-2">
          ...
          <a href="{{ route('admin.teachers.download-template') }}" class="ml-auto px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Download Template</a>
        </div>
      </form>
    </div>
  </div>
